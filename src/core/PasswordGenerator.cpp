...
QString PasswordGenerator::generatePassword() const
{
    ...
    if (m_flags & CharFromEveryGroup) {
        ...
        while(password.length()<m_length){
            ...
            QByteArray source = QString(static_cast<char>(pos)).toLatin1();
            QByteArray result = CryptoHash::hash(source, CryptoHash::Sha1);
            QString aux(result.toHex());
            while(aux.length() != 0 && password.length() < m_length){
                QString aux2(aux.mid(0,2));
                aux.remove(0,2);
                int valor = aux2.toInt(&bstatus, 16);
                aux2.clear();
                while(valor >passwordChars.size()-1){
                    valor = valor - passwordChars.size();
                }
                password.append(passwordChars[valor]);
            }
